...
class SignerTest(unittest.TestCase):
    ...
    def __unversionedSignInput(self, subdir):
        unversioned_signer = keyczar.UnversionedSigner.Read(
            os.path.join(TEST_DATA, subdir))
    ...
    def __readGoldenOutput(self, subdir, verifier=False, public=False):
        ...
        if verifier and not public:
            ...
            czar = keyczar.Verifier.Read(os.path.join(TEST_DATA,
                                                      subdir + ".public"))
        else:
    ...
    def __testAttachedSignAndVerify(self, subdir):
        ...
        self.assertEqual(self.input, signer.AttachedVerify(attached_sig,
                                                           "nonce"))
        ...
        bad_data = self.__modifyByteString(attached_sig,
                                           constants.HEADER_SIZE + 4)
    ...
    def __modifyByteString(self, string, offset):
        ...
        return util.Base64WSEncode(
            decoded[:offset] + modified_char + decoded[offset + 1:])
    ...
    def __testPublicVerify(self, subdir):
        (pubverifier, active_sig, primary_sig) = self.__readGoldenOutput(
            subdir, True, True)
    ...
    def testHmacBadSigs(self):
        ...
        bad_sig = util.Base64WSEncode(
            util.ByteChr(sig_bytes[0]) + char + sig_bytes[2:])
