...
TEST_DATA = os.path.realpath(os.path.join(os.getcwd(), "test-data",
                                          "existing-data", "python"))


class BaseCrypterTest(unittest.TestCase):

    ALL_BUFFER_SIZES = (util.DEFAULT_STREAM_BUFF_SIZE, 999, 1, -1, )

    def setUp(self):
        self.input_data = "This is some test data"
        self.random_input_data = os.urandom(random.randrange(
            util.DEFAULT_STREAM_BUFF_SIZE * 2 + 1, 10000))
    ...
    def __writeToStream(self, stream, data, len_to_write=-1):
        ...
        if len_to_write == -1:
            ...
            seg_data = [data[x:x + len_to_write]
                        for x in range(0, len(data), len_to_write)]
            for c in seg_data:
    ...
    def __testDecryptStream(self, subdir, reader, input_data,
                            stream_buffer_size, len_to_read, stream_source):
    ...
    def __testStreamEncryptAndStreamDecrypt(self, subdir, input_data,
                                            stream_buffer_size, len_to_rw,
                                            stream_source):
        crypter = keyczar.Crypter.Read(os.path.join(TEST_DATA, subdir))
        ciphertext_stream = io.BytesIO()
        if stream_source is None:
    ...
    def testAesKeyBadJavaLargeSizeHashDecrypt(self):
        self.__testDecrypt(os.path.join("..", "..", "special-case", "badhash",
                                        "aes-java-size"))
    ...
    def testAesEncryptedKeyDecrypt(self):
        file_reader = readers.FileReader(os.path.join(TEST_DATA,
                                                      "aes-crypted"))
    ...
    def testAesStandardEncryptAndStreamDecryptInterop(self):
        self.__testAllModesAndBufferSizes(
            self.__testStandardEncryptAndStreamDecrypt, ("aes", ))

    def testAesStreamEncryptAndStandardDecryptInterop(self):
        self.__testAllModesAndBufferSizes(
            self.__testStreamEncryptAndStandardDecrypt, ("aes", ))
    ...
    def testBadAesCiphertexts(self):
        ...
        ciphertext = util.Base64WSEncode(
            ciphertext[:2] + char + ciphertext[3:])
    ...
    def testKeysizeInterop(self):
        ...
        for size in keyinfo.AES.sizes:
            ...
            self.assertEqual(
                ...
                'Cannot encrypt/decrypt with the same PyCrypto key! size:%s' %
                size)
            ...
            self.assertEqual(
                self._encryptDecryptWithKey(s, aeskey), s,
                'Cannot encrypt/decrypt with the same M2Crypto key! size:%s' %
                size)
...
class CreateReaderTest(unittest.TestCase):
    ...
    def testFileReaderDoesNotOpenNonExistantLocation(self):
        ...
        self.assertRaises(errors.KeyczarError, readers.CreateReader, location)
...
class CreateWriterTest(unittest.TestCase):
    ...
    def testFileWriterDoesNotOpenNonExistantLocation(self):
        location = '/tmp/12312j3l'
        self.assertFalse(os.path.isdir(location))
        self.assertRaises(errors.KeyczarError, writers.CreateWriter, location)
