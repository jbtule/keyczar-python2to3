...
class BaseInteropTest(unittest.TestCase):
    ...
    def __init__(self, imp, methodname='runTest'):
        ...
        self.TEST_DATA = os.path.realpath(os.path.join(
            os.getcwd(), "test-data", "interop-data", imp + "_data"))
    ...
    def __testVerifySizes(self, subdir, size):
        (verifier, active_sig) = self.__readGoldenOutputSizes(subdir + "-size",
                                                              size)
        self.assertTrue(verifier.Verify(self.input, active_sig))
    ...
    def __testPublicVerifySizes(self, subdir, size):
        (pubverifier, active_sig) = self.__readGoldenOutputSizes(
            subdir + "-size", size, True)
        self.assertTrue(pubverifier.Verify(self.input, active_sig))
...
class FullInteropTest(BaseInteropTest):
    ...
    def __testSignedSessionDecrypt(self, subdir, signsubdir):
        ...
        material = util.ReadFile(os.path.join(path,
                                              "2.signedsession.material"))
        ciphertext = util.ReadFile(os.path.join(path,
                                                "2.signedsession.ciphertext"))
        crypter = keyczar.Crypter.Read(path)
    ...
    def __testVerifyTimeout(self, subdir, expired=False, public=False):
        ...
        def date():
            return datetime.datetime(2012, 12, 21, 11, 6)
        if expired:
            def date():
                return datetime.datetime(2012, 12, 21, 11, 16)
