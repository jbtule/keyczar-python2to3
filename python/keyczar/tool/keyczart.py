...
def main(argv):
    if len(argv) == 0:
        ...
        if cmd == CREATE:
            purpose = {
                'crypt': keyinfo.DECRYPT_AND_ENCRYPT,
                'sign': keyinfo.SIGN_AND_VERIFY
            }.get(flags.get(PURPOSE))
            Create(loc, flags.get(NAME, 'Test'), purpose,
            ...
            if CRYPTER2 in flags:
                ...
                crypter2 = None
            UseKey(flags.get(FORMAT), loc, flags.get(DESTINATION), crypter,
                   flags.get(DESTINATION2), flags.get(LOCATION2), crypter2,
                   other[1], other[2:])
        else:
            Usage()
