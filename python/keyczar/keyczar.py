...
class Encrypter(Keyczar):
    ...
    def CreateEncryptingStreamWriter(
            self, output_stream,
...
class Verifier(Keyczar):
    ...
    def __InternalVerify(self, header, signature, data, nonce=None):
        ...
        for key in matchedkeys:
            if key.Verify(
                    data + util.PackByteArray(nonce) + constants.VERSION_BYTE,
                    signature):
                return True
        return False
...
class Crypter(Encrypter):
    ...
    def CreateDecryptingStreamReader(
            self, output_stream,
...
class Signer(Verifier):
    ...
    """
        return util.Base64WSEncode(
            self.primary_key.Header() + util.PackByteArray(util.RawBytes(data)) +
...
class TimeoutSigner(TimeoutVerifier):
    ...
    """
        ...
        return util.Base64WSEncode(
            self.primary_key.Header() + expire_bytes +
            self.__InternalSign(expire_bytes + util.RawBytes(data)))
...
class _Session(object):
  ...
  """
    ...
    """
        return util.PackMultipleByteArrays(
            self.__session_key.key_bytes,
...
class SignedSessionDecrypter(object):
  ...
  """
    ...
    def __init__(self, crypter, verifier, session_material):
        self._session = _Session.LoadJsonSession(
            crypter.Decrypt(session_material))
