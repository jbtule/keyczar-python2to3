...
def ParsePkcs8(pkcs8):
    ...
    if oid == RSA_OID:
        ...
        version = int(key[0])
        if version != 0:
            raise kzr_errors.KeyczarError(
                "Unrecognized RSA Private Key Version")
...
def ExportDsaPkcs8(params):
    ...
    for i in range(len(DSA_PARAMS)):
        alg_params.setComponentByPosition(i,
                                          univ.Integer(params[DSA_PARAMS[i]]))
...
def ParseX509(x509):
    ...
    pubkey = decoder.decode(
        univ.OctetString(BinToBytes(seq[1].prettyPrint()[2:-3])))[0]
    ...
    params = {}
    if oid == RSA_OID:
        [params['n'], params['e']] = [long(x)
                                      for x in ParseASN1Sequence(pubkey)]
    elif oid == DSA_OID:
...
def MakeEmsaMessage(msg, modulus_size):
    ...
    magic_sha1_header = bytearray([0x30, 0x21, 0x30, 0x9, 0x6, 0x5, 0x2b, 0xe,
...
class BufferedIncrementalBase64WSEncoder(codecs.BufferedIncrementalEncoder):
  ...
  """
    ...
    @rtype: tuple
    """
        result = super(BufferedIncrementalBase64WSEncoder, self).encode(
            input,
            final=final)
...
class BufferedIncrementalBase64WSDecoder(codecs.BufferedIncrementalDecoder):
  ...
  """
    ...
    @rtype: string
    """
        result = super(BufferedIncrementalBase64WSDecoder, self).decode(
            input,
            final=final)
...
def ImportAll(pluginpath):
  ...
  """
    if os.path.exists(pluginpath):
        ...
        imported_modules = [__import__('%s' % (fname))
