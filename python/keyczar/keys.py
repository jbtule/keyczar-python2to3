...
class AesKey(SymmetricKey):
    ...
    def _FallbackHashes(self):
        ...
        if len(self.key_bytes) != 16:
            badjavahash = util.Hash(util.IntToBytes(16), self.key_bytes,
                                    self.hmac_key.key_bytes)
            fbh.append(
                util.Base64WSEncode(badjavahash[:constants.KEY_HASH_SIZE]))
        ...
        if len(self.key_bytes) > 0 and self.key_bytes[0] == b'\x00' [0]:
            ...
            badcpphash = util.Hash(util.IntToBytes(len(stripped_key_bytes)),
                                   stripped_key_bytes, self.hmac_key.key_bytes)
            fbh.append(
                util.Base64WSEncode(badcpphash[:constants.KEY_HASH_SIZE]))
...
class RsaPrivateKey(PrivateKey):
    ...
    def __Decode(self, encoded_message, label=b""):
        ...
        if diff_len > 1:
            ...
            encoded_message = util.RepeatByte(0x00,
                                              (diff_len - 1)) + encoded_message
    ...
    def __str__(self):
        return json.dumps({
            "publicKey": json.loads(str(self.public_key)),
            "privateExponent": util.Base64WSEncode(
                self.params['privateExponent']),
            "primeP": util.Base64WSEncode(self.params['primeP']),
            "primeQ": util.Base64WSEncode(self.params['primeQ']),
            "primeExponentP": util.Base64WSEncode(
                self.params['primeExponentP']),
            "primeExponentQ": util.Base64WSEncode(
                self.params['primeExponentQ']),
            "crtCoefficient": util.Base64WSEncode(
                self.params['crtCoefficient']),
            "size": self.size
        })
    ...
    def Generate(size=keyinfo.RSA_PRIV.default_size):
        ...
        params = {
            'privateExponent': util.BigIntToBytes(key.d),
            'primeP': util.BigIntToBytes(key.q),
            'primeQ': util.BigIntToBytes(key.p),
            'primeExponentP': util.BigIntToBytes(key.d % (key.q - 1)),
            'primeExponentQ': util.BigIntToBytes(key.d % (key.p - 1)),
            'crtCoefficient': util.BigIntToBytes(key.u)
        }
        pubkey = key.publickey()
        pub_params = {
            'modulus': util.BigIntToBytes(key.n),
...
class RsaPublicKey(PublicKey):
    ...
    def __Encode(self, msg, label=b""):
        ...
        datablock = label_hash + util.RepeatByte(0x00,
                                                 pad_octets) + b'\x01' + msg
        ...
        return json.dumps(
            {
                ...
                "publicExponent": util.Base64WSEncode(
                    self.params['publicExponent']),
                "size": self.size
            })
    ...
    def EncryptIO(self, reader, writer):
        ...
        writer.write(self.key.encrypt(data,
                                      None)[0])  # PyCrypto returns 1-tuple
        writer.flush()
...
class DecryptingStreamReader(object):
    ...
    def __CreateKey(self):
        ...
        if not self.__key:
            read_bytes, is_data_avail = self.__ReadBytes(
                constants.HEADER_SIZE - len(self.__encrypted_buffer))
            if read_bytes:
                self.__encrypted_buffer += read_bytes
            ...
            if len(self.__encrypted_buffer) >= constants.HEADER_SIZE:
                ...
                if self.__buffer_size >= 0:
                    self.__buffer_size = self.__key._NoPadBufferSize(
                        self.__buffer_size)
