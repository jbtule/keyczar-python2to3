...
parser.add_option("-y", "--assume_yes",
                  action="store_true",
                  dest="assume_yes",
                  default=False,
...
group.add_option("-f", "--description_file",
                 action="store",
...
group.add_option("-i", "--issue",
                 type="int",
                 action="store",
                 metavar="ISSUE",
                 default=None,
                 help="Issue number to which to add. Defaults to new issue.")
group.add_option("--download_base",
                 action="store_true",
                 dest="download_base",
                 default=False,
                 help="Base files will be downloaded by the server "
                 "(side-by-side diffs may not work on files with CRs).")
group.add_option("--rev",
                 action="store",
                 dest="revision",
                 metavar="REV",
                 default=None,
                 help="Branch/tree/revision to diff against (used by DVCS).")
group.add_option("--send_mail",
                 action="store_true",
                 dest="send_mail",
                 default=False,
  ...
  """
    ...
    if host == "localhost" or host.startswith("localhost:"):
        ...
        server = rpc_server_class(
            options.server, lambda: (email, "password"),
...
class SubversionVCS(VersionControlSystem):
    ...
    def _CollapseKeywords(self, content, keyword_str):
        ...
        keywords = [keyword for name in keyword_str.split(" ") for keyword in
                    svn_keywords.get(name, [])]
        return re.sub(r"\$(%s):(:?)([^\$]+)\$" % '|'.join(keywords), repl,
                      content)
    ...
    def GetStatus(self, filename):
        ...
        if not self.options.revision:
            ...
            if (len(status_lines) == 3 and not status_lines[0] and
...
class GitVCS(VersionControlSystem):
    ...
    def GetUnknownFiles(self):
        status = RunShell(["git", "ls-files", "--exclude-standard",
                           "--others"],
                          silent_ok=True)
    ...
    def GetBaseFile(self, filename):
        ...
        if hash == "0" * 40:  # All-zero hash indicates no base file.
            ...
            status = "M"
            base_content, returncode = RunShellWithReturnCode(["git", "show",
                                                               hash])
            if returncode:
...
class MercurialVCS(VersionControlSystem):
    ...
    def __init__(self, options, repo_dir):
        ...
        if self.options.revision:
            ...
            self.base_rev = RunShell(["hg", "parent",
                                      "-q"]).split(':')[1].strip()
    ...
    def GetBaseFile(self, filename):
        ...
        if status != "A":
            base_content = RunShell(["hg", "cat", "-r", self.base_rev,
                                     oldrelpath],
                                    silent_ok=True)
