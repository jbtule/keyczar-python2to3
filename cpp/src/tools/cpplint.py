...
threading_list = (
    ...
    ('getlogin(', 'getlogin_r('), ('getpwnam(', 'getpwnam_r('),
    ('getpwuid(', 'getpwuid_r('), ('gmtime(', 'gmtime_r('),
    ('localtime(', 'localtime_r('), ('rand(', 'rand_r('),
    ('readdir(', 'readdir_r('), ('strtok(', 'strtok_r('),
    ('ttyname(', 'ttyname_r('), )
...
def CheckPosixThreading(filename, clean_lines, linenum, error):
  ...
  """
    ...
    for single_thread_function, multithread_safe_function in threading_list:
        ...
        if ix >= 0 and (ix == 0 or
                        (not line[ix - 1].isalnum() and line[ix - 1] not in
                         ('_', '.', '>'))):
...
_RE_PATTERN_IVALID_INCREMENT = re.compile(r'^\s*\*\w+(\+\+|--);')
...
def CheckForNonStandardConstructs(filename, clean_lines, linenum, class_state,
                                  error):
  ...
  """
    ...
    if Search(r'\b(const|volatile|void|char|short|int|long'
              ...
              r'\s+(auto|register|static|extern|typedef)\b', line):
    ...
    if (args and args.group(1) != 'void' and
...
def CheckSpacing(filename, clean_lines, linenum, error):
  ...
  """
    ...
    if IsBlankLine(line):
        ...
        if (prevbrace != -1 and prev_line[prevbrace:].find('}') == -1 and
                prev_line[:prevbrace].find('namespace') == -1):
        ...
        if linenum + 1 < clean_lines.NumLines():
            ...
            if (next_line and Match(r'\s*}', next_line) and
                    next_line.find('namespace') == -1 and
  ...
  """
    ...
    if Search(r'}\s*else[^{]*$', line) or Match(r'[^}]*else\s*{', line):
        if Search(r'}\s*else if([^{]*)$', line):  # could be multi-line if
            ...
            pos = line.find('(', pos)
            if pos > 0:
                (endline, _, endpos) = CloseExpression(clean_lines, linenum,
                                                       pos)
    ...
    prevlinenum = linenum
    while True:
        (prevline, prevlinenum) = GetPreviousNonBlankLine(clean_lines,
                                                          prevlinenum)
        if Match(r'\s+{.*}\s*;', line) and not prevline.count(';'):
            line = prevline + line
        else:
            break
    if (Search(r'{.*}\s*;', line) and line.count('{') == line.count('}') and
...
def CheckLanguage(filename, clean_lines, linenum, file_extension,
  ...
  """
    ...
    if match:
        ...
        if not Match(r'^\s*MOCK_(CONST_)?METHOD\d+(_T)?\(', line):
            error(filename, linenum, 'readability/casting', 4,
                  ...
                  'Use static_cast<%s>(...) instead' % match.group(1))
    ...
    match = Match(
        r'((?:|static +)(?:|const +))string +([a-zA-Z0-9_:]+)\b(.*)', line)
...
def CheckCStyleCast(filename, linenum, line, raw_line, cast_type, pattern,
  ...
  """
    ...
    if function_match:
        if (not function_match.group(3) or function_match.group(3) == ';' or
                raw_line.find('/*') < 0):
...
_HEADERS_CONTAINING_TEMPLATES = (
    ...
    ('<functional>',
     ('unary_function', 'binary_function', 'plus', 'minus', 'multiplies',
      ...
      'mem_fun_ref', )), ('<limits>', ('numeric_limits', )), ('<list>',
                                                              ('list', )),
    ('<map>', ('map', 'multimap', )), ('<memory>', ('allocator', )),
    ('<queue>',
     ('queue', 'priority_queue', )), ('<set>',
                                      ('set', 'multiset', )), ('<stack>',
                                                               ('stack', )),
    ('<string>',
     ('char_traits', 'basic_string', )), ('<utility>',
                                          ('pair', )), ('<vector>',
                                                        ('vector', )),
    # gcc extensions.
    # Note: std::hash is their hash, ::hash is our hash
    ('<hash_map>', ('hash_map', 'hash_multimap', )
     ), ('<hash_set>', ('hash_set', 'hash_multiset', )), ('<slist>',
                                                          ('slist', )), )
...
def ProcessLine(filename, file_extension, clean_lines, line, include_state,
  ...
  """
    ...
    CheckLanguage(filename, clean_lines, line, file_extension, include_state,
                  error)
    CheckForNonStandardConstructs(filename, clean_lines, line, class_state,
                                  error)
