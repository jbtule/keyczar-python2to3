...
class dblite:
    ...
    def __setitem__(self, key, value):
        ...
        if (not is_string(key)):
            raise TypeError, "key `%s' must be a string but is %s" % (
                key, type(key))
...
def _exercise():
    ...
    try:
        ...
        assert str(
            e) == "key `(1, 2)' must be a string but is <type 'tuple'>", str(e)
    else:
    ...
    try:
        ...
        assert str(
            e) == "value `[1, 2]' must be a string but is <type 'list'>", str(e)
    else:
        raise RuntimeError, "TypeError exception expected"
    db = open("tmp", "r")
    assert len(db) == 5
    ...
    try:
        ...
        assert str(
            e) == "[Errno 2] No such file or directory: 'tmp.dblite'", str(e)
    else:
        raise RuntimeError, "IOError expected."
    print "OK"
