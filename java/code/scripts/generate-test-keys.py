...
keyFiles = [
    (testDataPrefix + "aes/", "crypt", None, None),
...
pubKeyFiles = [
    (testDataPrefix + "dsa/", testDataPrefix + "dsa.public/"),
...
def createFlags(loc,
    ...
    if asymmetric is not None:
        flags += "--asymmetric=" + asymmetric + " "
    if crypter is not None:
        flags += "--crypter=" + crypter + " "
    return flags[:-1]


def create(loc, purpose, name=None, asymmetric=None):
    args = createFlags(name=name,
                       loc=loc,
                       purpose=purpose,
